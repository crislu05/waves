...
def solve_coupled_heat_pump(t_span, T_initial, voltage_func, params, rtol, atol):
	"""
	...
	def heat_pump_rhs(t, State, params):
		...
		for i in range(1, len(T_brass) - 1):
			dT_brass_dt[i] = diff_coeff * (T_brass[i+1] - 2*T_brass[i] + T_brass[i-1]) / dx**2
...
def plot_temperatures_vs_time(sol, timestamp, thermistor_0, x_grid, thermistor_x_pos):
	"""
	Plot T_brass at thermistor position and thermistor_0 temperature as a function of time.
	...
	# Extract brass temperatures at all nodes
	...
	# Plot T_brass at thermistor position vs Time
	ax.plot(t_eval, T_brass_thermistor_C, 'g-', linewidth=2, 
	        label=f'T_brass (Model, x={thermistor_x_pos*1000:.1f}mm)')
	
	# Plot thermistor_0 vs Time (experimental data, already in Celsius)
	ax.plot(timestamp, thermistor_0, 'b-', linewidth=2, label='Thermistor 0 (Experimental)', alpha=0.7)
	
	ax.set_xlabel('Time (s)', fontsize=12)
	ax.set_ylabel('Temperature (°C)', fontsize=12)
	ax.set_title(f'Brass Temperature at x={thermistor_x_pos*1000:.1f}mm vs Time', fontsize=14, fontweight='bold')
...
def main():
	...
	h_hot = 50  # W/(m²·K) (Convective heat transfer coefficient for hot plate with fan)
	
	# Hot side surface area: Large finned heat sink dimensions 135 × 105 × 85 mm
	# Estimate total surface area including fins (typically 5-10x base area for finned heat sinks)
	heat_sink_length = 0.135  # m (135 mm)
	heat_sink_width = 0.105  # m (105 mm)
	heat_sink_height = 0.085  # m (85 mm)
	base_area = heat_sink_length * heat_sink_width  # m²
	A_hot = 8.0 * base_area  # m² (total surface area of finned heat sink, ~0.11 m²)
	...
	print(f"\nHot Side Heat Sink:")
	print(f"  Dimensions: {heat_sink_length*1000:.0f} × {heat_sink_width*1000:.0f} × {heat_sink_height*1000:.0f} mm")
